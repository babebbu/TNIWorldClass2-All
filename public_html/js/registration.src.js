...
function nextStep(){
    ...
    else if(stepNumber == 7)
    {
        ...
        {
            $.ajax({
                'url' : 'https://tniworldclass.com/registration/MTQuest',
                'type' : "POST",
            ...
            if(CKEDITOR.instances.question1.getData() == "")
                check += "กรุณากรอกคำถามที่ 1";
            if($('#uploadMT').text() != "อัพโหลดไฟล์แล้ว")
                check += "กรุณาอัพโหลดไฟล์";
        ...
        'success' : function(data){
            ...
            else if(facultySelect == "INT")
            {
                for(var i = 0 ;i<data.answer.length;i++)
                {
                    ...
                    ///
                    ///
                    else if(data.answer[i].question_id =="INT-2")
                        $('#uploadMT').html(function(){
                            return "อัพโหลดไฟล์แล้ว";
                        });
                    ///
    ...
    $.ajax({
        ...
        'success' : function(data)
        {
            ...
            else if(data.major.faculty == "INT")
            {
                ...
                $('#A2').html(function(){
                    return '<div class="col-sm-12" style="background : rgba(255,255,255,0.3); border-radius:10px; display:table;"> <p>'+data.answer_Faculty[1].answer+'</p></div>';
                });
                $('#A3').html(function(){
